...
/// Which of the three protocols an instrument implements, decided by the factory
/// that accepted the type rather than declared beside it.
...
/// Two independent reasons an instrument may not run on the device in hand: the
/// hardware it observes is absent from a simulator, and the API its mechanism
/// needs postdates this OS. The device gates activation on this; the menu renders
/// it, so a caller knows before it asks.
public struct Availability: Codable, Sendable, Equatable {
    ...
    /// A fleet spans OS versions and both simulators and hardware, so this is
    /// answered per device rather than decided when the catalog is built.
...
/// Everything about an instrument that no code on the device reads: what it is
/// for, how it works, what it costs, and how hard it has to be tested.
///
/// A value rather than a protocol, so it crosses into the modules that render it
/// without dragging `Instrument` along, and so the prose is one table read by the
/// menu emitter rather than 46 conformances gated on a compile-time flag.
public struct InstrumentMetadata: Codable, Sendable, Equatable {
    public enum Mechanism: String, Codable, Sendable {
        ...
        case osLogStore
        case metricKit
        /// Reading a framework's internals by name — `class_getInstanceVariable`,
        /// `object_getIvar`, `Mirror` over a private type. Ships, because no
        /// private symbol is linked or referenced. Fails to *silence* rather than
        /// to an error, so an instrument on this mechanism reports whether the
        /// read succeeded alongside what it read.
        case reflection
    }
    ...
    /// Which runs a reading describes.
    ///
    /// Almost everything here observes the process it is running inside.
    /// MetricKit does not: its payloads are daily aggregates about runs that have
    /// already ended, so ordering one expecting this run gets a correct answer to
    /// a question nobody asked. The menu carries this so that mistake is not
    /// available.
    ...
    var metadata: InstrumentMetadata {
        ...
        case .threadInventory:
            InstrumentMetadata(
                ...
                reveals: [
                    ...
                    "threads the scheduler has boosted above the priority they asked for, which is what priority donation looks like from outside",
                ],
                related: [
                    "cpu.thread.usage",
                    "runtime.threadSnapshot",
        ...
        case .cpuWakeups:
            InstrumentMetadata(
                ...
                reveals: [
                    "wakeups over the window just closed, rather than since the process started",
                    "the subset that pulled the platform out of idle, which is the expensive kind and the one iOS enforces a limit against",
                ],
                related: [
                    "cpu.thread.usage",
                    "power.thermalState",
        ...
        case .cloudKitEvents:
            InstrumentMetadata(
                summary: "What CloudKit sync is doing for this user, and what it failed with",
                whenToUse: "data does not appear on a user's other devices, or appears late — before assuming the app is at fault, since most sync failures are not",
                reveals: [
                    "every setup, import and export the container performs, and how long each took",
                    "which of them failed, with the CKError domain and code that says whether the user is signed out or out of storage",
                    "an event that started and has not finished, reported as still running rather than as a failure",
                ],
                related: [
                    "sync.keyValueStore",
                    "storage.coreData.save",
        ...
        case .keyValueStoreSync:
            InstrumentMetadata(
                summary: "Why the iCloud key-value store changed, including the quota violation that silently stops it",
                whenToUse: "settings or small state do not follow a user between devices, or stop following at some point and never resume",
                reveals: [
                    "a quota violation, which is the app exceeding 1 MB or 1024 keys and the only announcement that syncing has stopped",
                    "an account change, which is a user signing out of iCloud and the store appearing empty to the app",
                    "how many keys changed, never which — a key name is the app's own vocabulary and routinely carries a user's identifiers",
                ],
                related: ["sync.cloudKit.events", "environment.locale"],
                mechanism: .notification,
                cadence: .onChange
            )
