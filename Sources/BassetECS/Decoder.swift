...
private struct PayloadReader {
    ...
    /// A count read off the wire is a claim about the bytes that follow, and the
    /// bytes that follow are bounded. Checking it against them before reserving
    /// anything keeps a frame that claims a billion components from asking for a
    /// billion components' worth of memory — and keeps a count above `Int.max`
    /// from trapping on the conversion.
