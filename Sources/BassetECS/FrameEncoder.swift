...
public struct FrameEncoder {
    /// One byte carries the length, so this is the format's ceiling rather than
    /// a policy.
    static let maxStringBytes = 255

    public init() {}

    /// Cut on a character boundary rather than on the 255th byte. Cutting on the
    /// byte left a multi-byte character in halves, and half a character is not
    /// UTF-8 — so a decoder refuses the string, and with it every frame in the
    /// batch it arrived in. One accented word in a log line took the whole
    /// capture with it.
