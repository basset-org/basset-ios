...
46 instruments across 23 domains.
...
| [`sync`](Sources/Basset/Instruments/Sync.swift) | `cloudKit.events` `keyValueStore` | `NSPersistentCloudKitContainer` events · `NSUbiquitousKeyValueStore` notification — sync failures, silent quota stop |
| [`uikit`](Sources/Basset/Instruments/UIKit.swift) | `viewController.appear` `view.layoutPass` | `viewDidAppear` and `layoutSubviews` swizzle — screen order, layout passes per second |
...
| [`runtime`](Sources/Basset/Instruments/Runtime.swift) | `threadSnapshot` | `task_threads` + `THREAD_STATE` walk — every stack, dSYM-resolvable |
