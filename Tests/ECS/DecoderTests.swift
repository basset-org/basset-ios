...
struct DecoderTests {
    ...
    /// A count is a claim about bytes that are not there. Believing one is how a
    ...
    /// A string longer than the format's byte ceiling is cut, and where it is
    /// cut decides whether the frame survives: half a character is not UTF-8,
    /// and a decoder that refuses it refuses everything sent with it.
